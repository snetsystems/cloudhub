...
func TestDeviceMappingsStore_MoveDeviceOrg(t *testing.T) {
	...
	t.Run("MoveToNewOrg_AtomicUpdate", func(t *testing.T) {
		err = s.MoveDeviceOrg(ctx, device.Hostname, newOrgID)
		if err != nil {
			t.Fatal("Failed to move device to new org:", err)
		}

		// Verify device in new organization
		moved, err := s.GetDevice(ctx, device.Hostname)
		if err != nil {
			t.Fatal("Failed to get moved device:", err)
		}
		if moved.OrgID != newOrgID {
			t.Fatalf("Device org not updated: got %s, want %s", moved.OrgID, newOrgID)
		}

		// Verify reverse mapping is also updated
		deviceToOrg, err := s.GetByHostname(ctx, device.Hostname)
		if err != nil {
			t.Fatal("Failed to get device-to-org after move:", err)
		}
		if deviceToOrg.OrgID != newOrgID {
			t.Fatalf("Device-to-org mapping not updated: got %s, want %s", deviceToOrg.OrgID, newOrgID)
		}

		// Verify alias mapping is also updated
		aliasToDevice, err := s.GetByAlias(ctx, device.AliasName)
		if err != nil {
			t.Fatal("Failed to get alias-to-device after move:", err)
		}
		if aliasToDevice.OrgID != newOrgID {
			t.Fatalf("Alias-to-device mapping not updated: got %s, want %s", aliasToDevice.OrgID, newOrgID)
		}

		t.Log("All 3 keys are atomically updated during organization move")
	})
}

// TestDeviceMappingsStore_AliasOperations tests alias management
func TestDeviceMappingsStore_AliasOperations(t *testing.T) {
	c, err := NewTestClient("etcd")
	if err != nil {
		t.Fatal(err)
	}
	...
	t.Run("AddAdditionalAlias", func(t *testing.T) {
		newAlias := "backup-server"
		err = s.AddAlias(ctx, newAlias, device.OrgID, device.Hostname)
		if err != nil {
			t.Fatal("Failed to add additional alias:", err)
		}

		// Verify new alias
		aliasToDevice, err := s.GetByAlias(ctx, newAlias)
		if err != nil {
			t.Fatal("Failed to get new alias:", err)
		}
		if aliasToDevice.Hostname != device.Hostname {
			t.Fatalf("New alias mapping incorrect: got %s, want %s", aliasToDevice.Hostname, device.Hostname)
		}

		// Verify existing alias still exists
		originalAlias, err := s.GetByAlias(ctx, device.AliasName)
		if err != nil {
			t.Fatal("Original alias should still exist:", err)
		}
		if originalAlias.Hostname != device.Hostname {
			t.Fatal("Original alias mapping corrupted")
		}

		t.Log("Additional alias creation successful")
	})
	...
	t.Run("UpdateAlias", func(t *testing.T) {
		...
		newDevice := &cloudhub.DeviceMeta{
			...
			AliasName:  "database-server",
			DeviceType: "VM",
			OrgID:      "org-123",
		}
		err = s.AddDevice(ctx, newDevice)
		if err != nil {
			t.Fatal("Failed to add new device:", err)
		}

		// Change alias to new device
		aliasToUpdate := "backup-server"
		err = s.UpdateAlias(ctx, aliasToUpdate, newDevice.OrgID, newDevice.Hostname)
		if err != nil {
			t.Fatal("Failed to update alias:", err)
		}

		// Verify change
		updatedAlias, err := s.GetByAlias(ctx, aliasToUpdate)
		if err != nil {
			t.Fatal("Failed to get updated alias:", err)
		}
		if updatedAlias.Hostname != newDevice.Hostname {
			t.Fatalf("Alias not updated: got %s, want %s", updatedAlias.Hostname, newDevice.Hostname)
		}

		t.Log("Alias update successful")
	})

	t.Run("DeleteAlias", func(t *testing.T) {
		aliasToDelete := "backup-server"
		err = s.DeleteAlias(ctx, aliasToDelete)
		if err != nil {
			t.Fatal("Failed to delete alias:", err)
		}

		// Verify deletion
		_, err = s.GetByAlias(ctx, aliasToDelete)
		if err == nil {
			t.Fatal("Alias should be deleted but still exists")
		}

		t.Log("Alias deletion successful")
	})
}
...
func TestDeviceMappingsStore_TransactionRollback(t *testing.T) {
	...
	t.Run("MoveDeviceOrg_RollbackOnOrgValidationFailure", func(t *testing.T) {
		// Test rollback when org change fails after existing key deletion
		orgMoveDevice := &cloudhub.DeviceMeta{
			IP:         "192.168.1.700",
			Hostname:   "org-move-test",
			AliasName:  "org-move-alias",
			DeviceType: "VM",
			OrgID:      "original-org",
		}

		// Create device
		err := s.AddDevice(ctx, orgMoveDevice)
		if err != nil {
			t.Fatal("Failed to add org move test device:", err)
		}

		// Save existing state - verify all 3 keys
		originalDevice, err := s.GetDevice(ctx, orgMoveDevice.Hostname)
		if err != nil {
			t.Fatal("Failed to get original device:", err)
		}

		originalDeviceToOrg, err := s.GetByHostname(ctx, orgMoveDevice.Hostname)
		if err != nil {
			t.Fatal("Failed to get original device-to-org:", err)
		}

		originalAlias, err := s.GetByAlias(ctx, orgMoveDevice.AliasName)
		if err != nil {
			t.Fatal("Failed to get original alias:", err)
		}

		t.Log("Original state confirmed before org move")

		// Attempt to move to invalid-org (should fail)
		// MoveDeviceOrg will delete existing org-device key then fail at validation
		t.Log("Attempting org move to invalid org (should fail after key deletion)...")
		err = s.MoveDeviceOrg(ctx, orgMoveDevice.Hostname, "invalid-org")
		if err == nil {
			t.Fatal("Move to invalid org should have failed")
		}
		t.Log("Org move correctly failed:", err)

		// Verify rollback - confirm deleted keys were restored
		// 1. Verify original device data is intact
		afterFailureDevice, err := s.GetDevice(ctx, orgMoveDevice.Hostname)
		if err != nil {
			t.Fatal("ROLLBACK FAILED: Device not found after failed org move - original org-device key was not restored:", err)
		}

		if !reflect.DeepEqual(*afterFailureDevice, *originalDevice) {
			t.Fatalf("ROLLBACK FAILED: Device data was corrupted after failed org move\nGot: %+v\nWant: %+v",
				*afterFailureDevice, *originalDevice)
		}

		// 2. Verify device-to-org mapping is intact
		afterFailureDeviceToOrg, err := s.GetByHostname(ctx, orgMoveDevice.Hostname)
		if err != nil {
			t.Fatal("ROLLBACK FAILED: Device-to-org mapping not found after failed org move:", err)
		}

		if !reflect.DeepEqual(*afterFailureDeviceToOrg, *originalDeviceToOrg) {
			t.Fatalf("ROLLBACK FAILED: Device-to-org mapping was corrupted\nGot: %+v\nWant: %+v",
				*afterFailureDeviceToOrg, *originalDeviceToOrg)
		}

		// 3. Verify alias mapping is intact
		afterFailureAlias, err := s.GetByAlias(ctx, orgMoveDevice.AliasName)
		if err != nil {
			t.Fatal("ROLLBACK FAILED: Alias mapping not found after failed org move:", err)
		}

		if !reflect.DeepEqual(*afterFailureAlias, *originalAlias) {
			t.Fatalf("ROLLBACK FAILED: Alias mapping was corrupted\nGot: %+v\nWant: %+v",
				*afterFailureAlias, *originalAlias)
		}

		t.Log("CRITICAL ROLLBACK VERIFIED: Deleted org-device key was restored after validation failure")
		t.Log("  - Original org-device mapping preserved")
		t.Log("  - Device-to-org mapping intact")
		t.Log("  - Alias mapping intact")
	})

	t.Run("UpdateDevice_OrgChange_RollbackTest", func(t *testing.T) {
		// Test rollback when org change in UpdateDevice (calls MoveDeviceOrg)
		updateOrgDevice := &cloudhub.DeviceMeta{
			IP:         "192.168.1.800",
			Hostname:   "update-org-test",
			AliasName:  "update-org-alias",
			DeviceType: "VM",
			OrgID:      "update-original-org",
		}

		err := s.AddDevice(ctx, updateOrgDevice)
		if err != nil {
			t.Fatal("Failed to add update org test device:", err)
		}

		// Save existing state
		originalDevice, err := s.GetDevice(ctx, updateOrgDevice.Hostname)
		if err != nil {
			t.Fatal("Failed to get original device for update test:", err)
		}

		t.Log("Testing org change via UpdateDevice with invalid org...")

		// org change patch (attempt to change to invalid-org)
		patch := &cloudhub.DeviceMeta{
			OrgID: "invalid-org", // org that will fail in MoveDeviceOrg
		}

		err = s.UpdateDevice(ctx, updateOrgDevice.Hostname, patch)
		if err == nil {
			t.Fatal("UpdateDevice with invalid org should have failed")
		}
		t.Log("UpdateDevice with org change correctly failed:", err)

		// Verify rollback
		afterFailureDevice, err := s.GetDevice(ctx, updateOrgDevice.Hostname)
		if err != nil {
			t.Fatal("ROLLBACK FAILED: Device not found after failed UpdateDevice org change:", err)
		}

		if !reflect.DeepEqual(*afterFailureDevice, *originalDevice) {
			t.Fatalf("ROLLBACK FAILED: Device was corrupted after failed org change via UpdateDevice\nGot: %+v\nWant: %+v",
				*afterFailureDevice, *originalDevice)
		}

		t.Log("ROLLBACK VERIFIED: UpdateDevice org change failure preserved original state")
	})
