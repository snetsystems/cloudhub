...
type Server struct {
	...
	AddonURLs   map[string]string `short:"u" long:"addon-url" description:"Support addon is [salt, aws, gcp, osp, k8s, swan, oncue, ipmi-secret-key, ai, nvidia-gpu, dev-mode, powerflex]. Actually, this is a key-value extensional option not only url but also any key-value. Refer to the following usage samples. E.g., via flags: '-u=salt:{url} -u=salt_config_path:{path} -u=aws:on[off] -u=gcp:on[off] -u=k8s:on[off] -u=swan:{url} -u=oncue:{port number} -u=ipmi-secret-key:{seed key} -u=ai:on[off] -u=nvidia-gpu:on -u=dev:on -u=powerflex:on'. E.g. via environment variable: 'export ADDON_URL=salt:{url},swan:{url}'" env:"ADDON_URL" env-delim:","`
	AddonTokens map[string]string `short:"k" long:"addon-tokens" description:"The token associated with addon [salt, swan, nvidia-gpu, dev-mode, powerflex]. E.g. via flags: '-k=salt:{token} -k=swan:{token} -k=nvidia-gpu:{token} -k=powerflex:{token}'. E.g. via environment variable: 'export ADDON_TOKENS=salt:{token},swan:{token},nvidia-gpu:{token},'" env:"ADDON_TOKENS" env-delim:","`
