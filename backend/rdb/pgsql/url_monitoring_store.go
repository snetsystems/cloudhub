...
// All returns all active url_monitoring records (without targets).
...
FROM url_monitoring WHERE delete_yn = false ORDER BY created_at DESC`

	rows, err := s.client.QueryContext(ctx, q)
	if err != nil {
		return nil, fmt.Errorf("url_monitoring all: %w", err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&m.ID, &m.OrgID, &m.CollectorServer,
			...
			return nil, fmt.Errorf("url_monitoring all scan: %w", err)
		}
		m.Targets = []cloudhub.URLMonitoringTarget{}
		result = append(result, m)
	}
	if err := rows.Err(); err != nil {
		return nil, fmt.Errorf("url_monitoring all rows: %w", err)
	}
	return result, nil
}
...
INSERT INTO url_monitoring (org_id, collector_server, created_at, updated_at)
VALUES ($1, $2, $3, $4)
RETURNING id`
		...
		if err := row.Scan(&m.ID); err != nil {
			return fmt.Errorf("url_monitoring insert: %w", err)
		}
...
FROM url_monitoring WHERE org_id = $1 AND delete_yn = false`
	...
	if err := row.Scan(&m.ID, &m.OrgID, &m.CollectorServer); err != nil {
		...
		return nil, fmt.Errorf("url_monitoring get: %w", err)
	}
...
func (s *URLMonitoringStore) GetByID(ctx context.Context, id string) (*cloudhub.URLMonitoring, error) {
	const q = `
SELECT id, org_id, collector_server
FROM url_monitoring WHERE id = $1 AND delete_yn = false`
	...
	if err := row.Scan(&m.ID, &m.OrgID, &m.CollectorServer); err != nil {
		...
		return nil, fmt.Errorf("url_monitoring get by id: %w", err)
	}
	targets, err := s.getTargets(ctx, m.ID)
	if err != nil {
		return nil, err
...
WHERE id = $1 AND delete_yn = false`
		...
		if err != nil {
			return fmt.Errorf("url_monitoring update: %w", err)
		}
...
func syncTargets(ctx context.Context, tx rdb.Store, m *cloudhub.URLMonitoring, now time.Time) error {
	if len(m.Targets) == 0 {
		_, err := tx.ExecContext(ctx,
			`UPDATE url_monitoring_targets SET delete_yn = true, updated_at = $2 WHERE url_monitoring_id = $1 AND delete_yn = false`,
			m.ID, now,
		)
		if err != nil {
			return fmt.Errorf("url_monitoring_targets soft-delete all: %w", err)
		}
		return nil
	}
	...
	rows, err := tx.QueryContext(ctx,
		`SELECT id FROM url_monitoring_targets WHERE url_monitoring_id = $1 AND delete_yn = false`,
		m.ID,
	)
	if err != nil {
		return fmt.Errorf("url_monitoring_targets list active: %w", err)
	}
	defer rows.Close()
	var active []string
	for rows.Next() {
		...
		if err := rows.Scan(&id); err != nil {
			return fmt.Errorf("url_monitoring_targets scan id: %w", err)
		}
	...
	for _, id := range active {
		...
		if _, err := tx.ExecContext(ctx,
			`UPDATE url_monitoring_targets SET delete_yn = true, updated_at = $2 WHERE id = $1 AND delete_yn = false`,
			id, now,
		); err != nil {
			return fmt.Errorf("url_monitoring_targets soft-delete removed: %w", err)
		}
	}
	...
	for i := range m.Targets {
		...
		const upd = `
UPDATE url_monitoring_targets SET
    name             = $2,
    url              = $3,
    interval         = $4,
    response_timeout = $5,
    ...
    alert_rule_id    = $7,
    updated_at       = $8
WHERE id = $1 AND url_monitoring_id = $9 AND delete_yn = false`
		res, err := tx.ExecContext(ctx, upd,
			t.ID, t.Name, t.URL, t.Interval, t.ResponseTimeout, t.Method, t.AlertRuleID, now, m.ID,
		)
		if err != nil {
			return fmt.Errorf("url_monitoring_targets update: %w", err)
		}
		if res.RowsAffected() == 0 {
			return fmt.Errorf("url_monitoring_targets update: target id %q not found for this monitoring", t.ID)
		}
	}
	return nil
}
...
func (s *URLMonitoringStore) Delete(ctx context.Context, id string) error {
	return s.client.WithTx(ctx, func(ctx context.Context, tx rdb.Store) error {
		if _, err := tx.ExecContext(ctx,
			`UPDATE url_monitoring_targets SET delete_yn = true, updated_at = now()
			 WHERE url_monitoring_id = $1 AND delete_yn = false`,
			id,
		); err != nil {
			return fmt.Errorf("url_monitoring_targets cascade soft-delete: %w", err)
		}
		result, err := tx.ExecContext(ctx,
			`UPDATE url_monitoring SET delete_yn = true, updated_at = now() WHERE id = $1 AND delete_yn = false`,
			id,
		)
		if err != nil {
			return fmt.Errorf("url_monitoring delete: %w", err)
		}
		if result.RowsAffected() == 0 {
			return cloudhub.ErrURLMonitoringNotFound
		}
...
// getTargets fetches active targets for the given url_monitoring ID.
func (s *URLMonitoringStore) getTargets(ctx context.Context, monitoringID string) ([]cloudhub.URLMonitoringTarget, error) {
	const q = `
SELECT id, url_monitoring_id, name, url, interval, response_timeout, method, alert_rule_id
FROM url_monitoring_targets
WHERE url_monitoring_id = $1 AND delete_yn = false
ORDER BY created_at ASC`

	rows, err := s.client.QueryContext(ctx, q, monitoringID)
	if err != nil {
		return nil, fmt.Errorf("url_monitoring_targets get: %w", err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&t.ID, &t.URLMonitoringID, &t.Name, &t.URL,
			...
			return nil, fmt.Errorf("url_monitoring_targets scan: %w", err)
		}
...
// insertTargets inserts a slice of targets for the given url_monitoring ID.
...
func insertTarget(ctx context.Context, tx rdb.Store, monitoringID string, t *cloudhub.URLMonitoringTarget, now time.Time) error {
	const q = `
INSERT INTO url_monitoring_targets (url_monitoring_id, name, url, interval, response_timeout, method, alert_rule_id, created_at, updated_at)
...
RETURNING id`
	...
	if err := row.Scan(&t.ID); err != nil {
		return fmt.Errorf("url_monitoring_targets insert: %w", err)
	}
	return nil
}
