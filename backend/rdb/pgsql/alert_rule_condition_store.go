...
func (s *AlertRuleConditionStore) ByRule(ctx context.Context, ruleID string) ([]cloudhub.AlertRuleCondition, error) {
	const q = `SELECT alert_rule_id, level, value, operator, enabled FROM alert_rule_conditions WHERE alert_rule_id = $1 ORDER BY level`
	...
	for rows.Next() {
		...
		if err := rows.Scan(&c.AlertRuleID, &c.Level, &c.Value, &c.Operator, &c.Enabled); err != nil {
			return nil, err
		}
	...
	return s.client.WithTx(ctx, func(ctx context.Context, tx rdb.Store) error {
		...
		for _, c := range conditions {
			...
			if _, err := tx.ExecContext(ctx,
				`INSERT INTO alert_rule_conditions (alert_rule_id, level, value, operator, enabled) VALUES ($1,$2,$3,$4,$5)`,
				ruleID, c.Level, c.Value, operator, c.Enabled,
			); err != nil {
				return err
			}
		}
