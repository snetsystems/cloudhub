...
func varWhereFilter(vars map[string]tick.Var) (WhereFilter, bool) {
	...
	// CloudHub TICKScripts use lambda: TRUE as a pass-through where clause
...
func extractCommonVars(vars map[string]tick.Var) (CommonVars, error) {
	...
	// All cloudhub TICKScripts must have a whereFilter.  Could be empty.
...
func TargetedReverseParser(script cloudhub.TICKScript, regex string) (cloudhub.AlertRule, error) {
	rule := cloudhub.AlertRule{
		Query: &cloudhub.QueryConfig{},
	}
	...
	detail, ok := varString("details", vars)
	if ok {
		rule.Details = detail
	}
