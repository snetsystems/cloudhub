...
def create_package_fs(build_root):
    ...
    """
    logging.debug(
        "Creating package filesystem at location: {}".format(build_root))
...
def run(command, allow_failure=False, shell=False, print_output=False):
    ...
    try:
        ...
        if p.returncode != 0:
            if allow_failure:
                logging.warn(
                    u"Command '{}' failed with error: {}".format(command, out))
                return None
            else:
                logging.error(
                    u"Command '{}' failed with error: {}".format(command, out))
...
def upload_packages(packages, bucket_name=None, overwrite=False):
    ...
    """
    logging.debug("Uploading files to bucket '{}': {}".format(
    ...
    for p in packages:
        ...
        if bucket.get_key(name) is None or overwrite:
            ...
            logging.warn(
                "Not uploading file {}, as it already exists in the target bucket.".format(name))
    return True
...
def generate_sig_from_file(path):
    ...
    if gpg_path is None:
        logging.warn(
            "gpg binary not found on path! Skipping signature creation.")
        return False
            ...
            for arch in build_output[platform]:
                ...
                for binary in targets:
                    ...
                    if platform == 'windows' or static or "static_" in arch:
                        ...
                        # Where the binary should go in the package filesystem
                        to = os.path.join(
                            build_root, INSTALL_ROOT_DIR[1:], binary)
                    shutil.copy(fr, to)
                ...
                for package_type in supported_packages[platform]:
                    ...
                    if package_type in ['zip', 'tar']:
                        ...
                        if package_type == 'tar':
                            tar_command = "cd {} && tar -cvzf {}.tar.gz --owner=root ./*".format(
                            ...
                            if nightly:
                                ...
                                if package_type == 'rpm':
                                    ...
                                    package_version = package_version.replace(
                                        "-", "_")
                                    logging.debug("Changing package output version from {} to {} for RPM.".format(
                                        version, package_version))
                                new_outfile = outfile.replace(
                                    "{}-{}".format(package_version, package_iteration), package_version)
                                os.rename(outfile, new_outfile)
                                outfile = new_outfile
...
def main(args):
    ...
    if args.package:
        ...
        for p in packages:
            ...
            if regex is None or type is None:
                logging.error(
                    "Could not determine package type for: {}".format(p))
                return 1
    ...
    if orig_branch != get_current_branch():
        logging.info(
            "Moving back to original git branch: {}".format(orig_branch))
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description='InfluxDB build and packaging script.')
