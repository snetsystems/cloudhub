...
export const mergeResultsByProcessName = (
  results: Array<{value: TimeSeriesResponse | null; error: unknown | null}>
) => {
  const rowMap = new Map<string, Record<string, HostCellValue>>()
  ...
  results.forEach(result => {
    ...
    response.results.forEach(statement => {
      ...
      statement.series.forEach(series => {
        const processName = series.tags?.process_name
        if (!processName) {
          return
        }
        const user = series.tags?.user ?? ''

        const rowKey = `${processName}|||${user}`
        const row =
          rowMap.get(rowKey) ||
          ({process_name: processName, user} as Record<string, HostCellValue>)
