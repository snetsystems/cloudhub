...
export enum ActionTypes {
  SetSource = 'ACTIVITYLOGS_SET_SOURCE',
  SetNamespaces = 'ACTIVITYLOGS_SET_NAMESPACES',
  SetTimeBounds = 'ACTIVITYLOGS_SET_TIMEBOUNDS',
  SetTimeWindow = 'ACTIVITYLOGS_SET_TIMEWINDOW',
  SetTimeMarker = 'ACTIVITYLOGS_SET_TIMEMARKER',
  SetNamespace = 'ACTIVITYLOGS_SET_NAMESPACE',
  SetHistogramQueryConfig = 'ACTIVITYLOGS_SET_HISTOGRAM_QUERY_CONFIG',
  SetHistogramData = 'ACTIVITYLOGS_SET_HISTOGRAM_DATA',
  SetTableQueryConfig = 'ACTIVITYLOGS_SET_TABLE_QUERY_CONFIG',
  SetTableData = 'ACTIVITYLOGS_SET_TABLE_DATA',
  AddFilter = 'ACTIVITYLOGS_ADD_FILTER',
  RemoveFilter = 'ACTIVITYLOGS_REMOVE_FILTER',
  ChangeFilter = 'ACTIVITYLOGS_CHANGE_FILTER',
  ClearFilters = 'ACTIVITYLOGS_CLEAR_FILTERS',
  IncrementQueryCount = 'ACTIVITYLOGS_INCREMENT_QUERY_COUNT',
  DecrementQueryCount = 'ACTIVITYLOGS_DECREMENT_QUERY_COUNT',
  ConcatMoreLogs = 'ACTIVITYLOGS_CONCAT_MORE_LOGS',
  PrependMoreLogs = 'ACTIVITYLOGS_PREPEND_MORE_LOGS',
  ReplacePrependedLogs = 'ACTIVITYLOGS_REPLACE_PREPENDED_LOGS',
  SetConfig = 'ACTIVITYLOGS_SET_CONFIG',
  SetTableRelativeTime = 'ACTIVITYLOGS_SET_TABLE_RELATIVE_TIME',
  SetTableCustomTime = 'ACTIVITYLOGS_SET_TABLE_CUSTOM_TIME',
  SetTableForwardData = 'ACTIVITYLOGS_SET_TABLE_FORWARD_DATA',
  SetTableBackwardData = 'ACTIVITYLOGS_SET_TABLE_BACKWARD_DATA',
  ClearRowsAdded = 'ACTIVITYLOGS_CLEAR_ROWS_ADDED',
  ClearTableData = 'ACTIVITYLOGS_CLEAR_TABLE_DATA',
  SetNextOlderUpperBound = 'ACTIVITYLOGS_SET_NEXT_OLDER_UPPER_BOUND',
  SetNextOlderLowerBound = 'ACTIVITYLOGS_SET_NEXT_OLDER_LOWER_BOUND',
  SetNextNewerUpperBound = 'ACTIVITYLOGS_SET_NEXT_NEWER_UPPER_BOUND',
  SetNextNewerLowerBound = 'ACTIVITYLOGS_SET_NEXT_NEWER_LOWER_BOUND',
  SetCurrentTailUpperBound = 'ACTIVITYLOGS_SET_CURRENT_TAIL_UPPER_BOUND',
  SetNextTailLowerBound = 'ACTIVITYLOGS_SET_NEXT_TAIL_LOWER_BOUND',
  SetSearchStatus = 'ACTIVITYLOGS_SET_SEARCH_STATUS',
}
...
const getIsTruncated = (state: State): boolean =>
  state.activitylogs.logConfig.isTruncated

const getForwardTableData = (state: State): TableData =>
  state.activitylogs.tableInfiniteData.forward

const getBackwardTableData = (state: State): TableData =>
  state.activitylogs.tableInfiniteData.backward
...
const getTimeRange = (state: State): TimeRange | null =>
  getDeep<TimeRange | null>(state, 'activitylogs.timeRange', null)

const getNamespace = (state: State): Namespace | null =>
  getDeep<Namespace | null>(state, 'activitylogs.currentNamespace', null)

const getProxyLink = (state: State): string | null =>
  getDeep<string | null>(state, 'activitylogs.currentSource.links.proxy', null)

const getHistogramQueryConfig = (state: State): QueryConfig | null =>
  getDeep<QueryConfig | null>(state, 'activitylogs.histogramQueryConfig', null)

const getTableQueryConfig = (state: State): QueryConfig | null =>
  getDeep<QueryConfig | null>(state, 'activitylogs.tableQueryConfig', null)

const getSearchTerm = (state: State): string | null =>
  getDeep<string | null>(state, 'activitylogs.searchTerm', null)

const getFilters = (state: State): Filter[] =>
  getDeep<Filter[]>(state, 'activitylogs.filters', [])

const getTableSelectedTime = (state: State): number => {
  const custom = getDeep<string>(state, 'activitylogs.tableTime.custom', '')

  if (!_.isEmpty(custom)) {
    return Date.parse(custom)
  }

  const relative = getDeep<number>(state, 'activitylogs.tableTime.relative', 0)
...
const getNextOlderUpperBound = (state: State): number => {
  ...
  return getDeep<number>(
    state,
    'activitylogs.nextOlderUpperBound',
    selectedTableTime
  )
}
...
const getOlderChunkDurationMs = (state: State): number => {
  return getDeep<number>(
    state,
    'activitylogs.olderChunkDurationMs',
    DEFAULT_OLDER_CHUNK_DURATION_MS
  )
}
...
const getNextNewerLowerBound = (state: State): number => {
  ...
  return getDeep<number>(
    state,
    'activitylogs.nextNewerLowerBound',
    selectedTableTime
  )
}
...
const getNewerChunkDurationMs = (state: State): number => {
  return getDeep<number>(
    state,
    'activitylogs.newerChunkDurationMs',
    DEFAULT_NEWER_CHUNK_DURATION_MS
  )
}

const getNextTailLowerBound = (state: State): number | void =>
  state.activitylogs.nextTailLowerBound
...
const getMaxTailBufferDurationMs = (state: State): number => {
  return getDeep<number>(
    state,
    'activitylogs.maxTailBufferDurationMs',
    DEFAULT_MAX_TAIL_BUFFER_DURATION_MS
  )
}
...
export const setHistogramQueryConfigAsync = () => async (
  ...
  const namespace = getDeep<Namespace | null>(
    state,
    'activitylogs.currentNamespace',
    null
  )
  const timeRange = getDeep<TimeRange | null>(
    state,
...
export const flushTailBuffer = () => (
  ...
  dispatch(setTableForwardData(defaultActivityTableData))
}
...
export const fetchNamespaceSyslogStatusAsync = (namespace: Namespace) => async (
  ...
  try {
    ...
    const response = await getActivitylogMeasurement(proxyLink, namespace)
...
export const getLogConfigAsync = (_url: string) => async (
  ...
  try {
    ...
    const data = {
      links: {self: '/cloudhub/v1/org_config/logviewer'},
      columns: [
        {
          name: 'time',
          position: 0,
          encodings: [{type: 'visibility', value: 'hidden'}],
        },
        {
          name: 'severity',
          position: 1,
          encodings: [
            ...
            {type: 'color', value: 'ruby', name: 'emerg'},
            {type: 'color', value: 'fire', name: 'alert'},
            {type: 'color', value: 'curacao', name: 'crit'},
            {type: 'color', value: 'tiger', name: 'err'},
            {type: 'color', value: 'pineapple', name: 'warning'},
            {type: 'color', value: 'rainforest', name: 'notice'},
            {type: 'color', value: 'star', name: 'info'},
            {type: 'color', value: 'wolf', name: 'debug'},
          ],
        },
        {
          name: 'timestamp',
          position: 2,
          encodings: [{type: 'visibility', value: 'visible'}],
        },
        {
          name: 'user',
          position: 3,
          encodings: [
            {type: 'visibility', value: 'visible'},
            {type: 'displayName', value: 'User ID'},
          ],
        },
        {
          name: 'message',
...
export const updateLogConfigAsync = (_url: string, config: LogConfig) => async (
  dispatch: Dispatch<SetConfigAction>
): Promise<void> => {
  try {
    logConfigUIToServer(config)
    // await updateLogConfigAJAX(_url, configForServer)
