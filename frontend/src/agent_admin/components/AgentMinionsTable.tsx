...
class AgentMinionsTable extends PureComponent<Props, State> {
	...
	public getSortedHosts = memoize((minions, searchTerm: string, sortKey: string, sortDirection: SortDirection) =>
	...
	public updateSort = (key) => () => {
		...
		if (sortKey === key) {
			const reverseDirection = sortDirection === SortDirection.ASC ? SortDirection.DESC : SortDirection.ASC
	...
	private get NoSortedHostsState(): JSX.Element {
		return (
			<div className="generic-empty-state">
				<h4 style={{ margin: '90px 0' }}>There are no hosts that match the search criteria</h4>
			</div>
		)
	}
	...
	public render() {
		...
		return (
			<div className="panel">
				{minionsPageStatus === RemoteDataState.Loading ? this.LoadingState : null}
				<div className="panel-heading">
					<h2 className="panel-title">{this.AgentTitle}</h2>
					<SearchBar placeholder="Filter by Host..." onSearch={this.updateSearchTerm} />
				</div>
		...
		return (
			<div className="hosts-table--thead">
				<div className="hosts-table--tr">
					...
					<div className="hosts-table--th list-type" style={{ width: StatusWidth }}>
						Operation
					</div>
				</div>
			</div>
	...
	private get AgentTableWithHosts() {
		const { minions, onClickTableRow, onClickModal, handleWheelKeyCommand, focusedHost } = this.props
		const { sortKey, sortDirection, searchTerm } = this.state
