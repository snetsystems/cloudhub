...
@ErrorHandling
class HostsPageContainer extends PureComponent<Props, State> {
  constructor(props: Props) {
    super(props)

    // Helper: restore template variables
    ...
    this.state = {
      ...
      autoRefreshOptions: getTimeOptionByGroup('host'),
  ...
  public handleChooseAutoRefresh = (option: {
    ...
    // Always update 'host' group for this container
    onChooseCloudAutoRefresh({host: milliseconds})
  }

  public componentDidMount() {
    // Show template control bar by default for Hosts
    const {
      showTemplateVariableControlBar,
      toggleTemplateVariableControlBar,
    } = this.props

    if (!showTemplateVariableControlBar) {
      toggleTemplateVariableControlBar()
    }
  }
  ...
  public render() {
    ...
    const {timeRange, templates, zoomedTimeRange} = this.state
    ...
    return (
      <Page className="hosts-list-page">
        <DashboardHeader
          ...
          onAddCell={() => {}} // Always enabled for Host
          onImportFromLibrary={this.handleShowImportOverlay} // Always enabled for Host
...
const mstp = state => {
  ...
  return {
    links,
    autoRefresh: cloudAutoRefresh?.host || 0, // Use Host-specific autoRefresh for this page
    cloudTimeRange,
    cloudAutoRefresh,
    inPresentationMode,
    showTemplateVariableControlBar,
...
export default connect(
  ...
  mdtp,
  null
)(ManualRefresh<Props>(HostsPageContainer))
